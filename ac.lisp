...
(defgeneric wfn (wkr args nrm arg-opt arg-rest syms body)
  (:method (wkr args nrm arg-opt arg-rest syms body)
    (declare (ignore syms))
    `(fn ,args ,@body)))
  ...
  (let (nrm opt rest syms)
    (labels ((_err ()
		 ...
		 (let ((a (car x)))
		   (cond ((null x) nil)
			 ((_opt? a)
			  (cons (_arg t a) (_argl t (cdr x))))
			 (o? (_err))
			 ((null (cdr x)) (list (_arg nil a)))
			 ((consp (cdr x))
			  (cons (_arg nil a) (_argl nil (cdr x))))
			 (t 
			  (push (cdr x) rest)
			  (push (cdr x) syms)
			  (cons (_arg nil a) (cdr x)))))))
	     (_args (a) 
	       (cond ((null a) nil)
		     ((symbolp a) 
		      (push a rest)
...
(defmethod wfn ((wkr c) args nrm arg-opt arg-rest syms body)
  (declare (ignore args))
  `(lambda (,@nrm
